...
public class AVLTreeComparable<Key extends Comparable<Key>, Value> {
    ...
    public AVLNodeComparable delete(AVLNodeComparable x, Key key) {
        int cmp = key.compareTo((Key) x.key);
        if (cmp < 0) {
    ...
    public String toString() {
        return "AVLTreeComparable{" +
                "root=" + root +
                '}';
    }
    public void traverseLevelOrder(AVLNodeComparable focus) {
        java.util.LinkedList<AVLNodeComparable> que = new java.util.LinkedList<AVLNodeComparable>();
        que.add(focus);
        while (!que.isEmpty()) {
            AVLNodeComparable d = que.removeFirst();
            if (d.left != null) {
                que.addLast(d.left);
            }
            if (d.right != null) {
